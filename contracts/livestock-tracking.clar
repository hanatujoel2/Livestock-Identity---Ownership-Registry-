...
(define-map livestock-registry
  uint
  {
    ...
    parent-ids: (optional {sire: uint, dam: uint}),
...
(define-map transfer-history
  {livestock-id: uint, transfer-id: uint}
  {
...
(define-public (register-livestock 
  ...
  (let 
    ...
    (asserts! (or (is-eq gender "male") (is-eq gender "female") (is-eq gender "unknown")) ERR_INVALID_INPUT)
...
(define-public (transfer-livestock 
  ...
  (let 
    ...
    (map-set transfer-history 
      {livestock-id: livestock-id, transfer-id: transfer-count}
      {
...
(define-public (add-health-record
  ...
  (let 
    ...
    (asserts! (or (is-eq tx-sender owner) (is-eq tx-sender veterinarian)) ERR_UNAUTHORIZED)
    (asserts! (> (len treatment-type) u0) ERR_INVALID_INPUT)
    
    ;; Add health record
    (map-set health-records 
      {livestock-id: livestock-id, record-id: health-count}
      {
...
(define-read-only (get-transfer-record (livestock-id uint) (transfer-id uint))
  (map-get? transfer-history {livestock-id: livestock-id, transfer-id: transfer-id})
)
